...
**9.2 — 貼這段 prompt 到 Lovable**（把 `<SUPABASE_URL>` 和 `<PUBLISHABLE_KEY>` 換成 9.1 的兩個值）：
...
**9.4 — 在正式的 Vercel 網址上驗證**（不是只有 Lovable 預覽）: 註冊一個全新的測試 email → 如果需要就點確認信 → 登入 → 檢查 Supabase **Authentication → Users** — 新用戶出現在**你自己**的專案裡（不是 Lovable Cloud）。

> 📌 **兩個 Supabase 值也一起快取起來。** AWS 已經在 Step 5 設好了，所以照 GitHub token 的做法：把 Project URL + publishable key 交給 Claude Code，讓它存一個 `flight/supabase` secret，之後的 session 就能直接讀，不用再去 dashboard 找。publishable key 本來就是設計成公開的（會出現在瀏覽器 bundle 裡），所以快取它只是方便；**絕對不要**快取 service-role key。（之後的 milestone 會用 `list-secrets` 掃描含 `supabase` 字樣的 secret 名稱來找到它 — 見 [[aws-best-practice]] Rule 2。）
